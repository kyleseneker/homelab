...
- [ ] envtest coverage for every controller. Reconciling against a real kube-apiserver catches what a fake client cannot: status is a separate subresource write, CRD validation rejects values the Go types accept, and a finalizer genuinely blocks deletion. `TdarrConfig`, `BazarrConfig`, `MaintainerrConfig`, `AutobrrConfig` and `SabnzbdConfig` cover status persistence and orphan deletion against a live apiserver, sharing one control plane across the table. The `pvr`, `indexers`, `mediaservers` and `requests` kinds still rely on fake-client tests, which sit between 19% and 64% statement coverage
