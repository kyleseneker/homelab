...
| K25 | **Recyclarr has never actually synced** | Sonarr reports **0 custom formats**, so the one piece of *arr config already in Git has never reached the apps. Bumping 8.5.1 → 8.7.0 removed the crash (`Could not find color or style '<revision>...'`, an 8.5.x bug rendering error messages with special characters), but sync now exits 0 while reporting `Found 0 config files with 0 Radarr and 0 Sonarr instances`. Ruled out: the `!secret` tag, the read-only subPath mounts, file ownership, and both the `configs/` and root layouts under an explicit `RECYCLARR_CONFIG_DIR`. Root cause still open. Separately, the Sonarr key in `recyclarr-secrets` returns 401 -- the same drift as K27, fixed by C1. | High |
