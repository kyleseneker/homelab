...
| K39 | **Nothing reconciles `k8s/bootstrap/`** | That directory is applied by `kubectl apply -k`, not by ArgoCD, so it can drift from git indefinitely with no detection. Proven today: the CA ConfigMap was updated and pushed to git, yet the cluster kept an expired copy until it was applied by hand. It holds the ArgoCD install, the ApplicationSet, and the CA bundle -- the pieces most needed for a rebuild. Fix: either let ArgoCD self-manage the directory, or add a job running `kubectl diff -k` and alerting on drift. | High |
