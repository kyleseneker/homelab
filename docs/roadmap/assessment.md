...
| K28 | **The control-plane node cannot create new pods** | `kubelet` on `homelabk8s01-node-1` fails every pod sandbox with `Failed to activate service 'org.freedesktop.systemd1': timed out` -- systemd/dbus is wedged. `sshd` is also down on that node while node-2/3 respond. The node reports `Ready` and its existing pods (etcd, kube-apiserver, controller-manager, scheduler, both CoreDNS) have run 122 days, but nothing new can start. If any static pod restarts it will not come back. Requires a node reboot via Proxmox or AMT. | Critical |
| K29 | **etcd backups stopped 2026-06-02 and nothing surfaced it** | `lastSuccessfulTime` is 61 days stale because the CronJob is pinned to the control plane (K28). `EtcdBackupStale` was firing correctly the whole time -- but Alertmanager could not deliver any notification, so no alert reached a human. K16 is therefore regressed, not Resolved. | Critical |
| K30 | **Alertmanager delivery was silently broken** | Every notification failed with `connect: operation not permitted` to the `openclaw` webhook receiver. Not a policy denial: Cilium's socket-LB returns `EPERM` when a ClusterIP has zero ready backends, and OpenClaw was CrashLoopBackOff (its image needed a writable `/home/node/.npm`). Because that receiver sits in the shared route tree, its retry exhaustion suppressed delivery for every alert. Fixed, but the single-receiver-blocks-all-alerts shape remains. | Resolved |
...
| K28, K29 | Unassigned -- needs a control-plane node reboot first |
