...
[Verified recovery checks](../runbooks/backup-and-restore.md) cover Sonarr's database/login and bounded operator, Recyclarr and Prowlarr reconciliation; Authentik's logical database, emergency login and OIDC exchange; Tdarr's native configuration and plugins; qBittorrent's configuration/resume metadata and fresh login across restart; and Vault's quiesced local/S3 file-backend restores, KMS auto-unseal, scoped Kubernetes auth/ESO recovery, lab migration/rollback, and production Raft cutover with an isolated restore of its native S3 snapshot. Daily native snapshots, read-back verification, scoped retention and freshness monitoring are deployed. The [isolated etcd/API drill](../runbooks/disaster-recovery.md#isolated-etcd-and-api-verification) also recovered a complete offsite snapshot/PKI/host-configuration bundle without querying production for component configuration, verified revision bump/compaction, and matched stored/API object counts. Daily backups now publish a completion manifest only after S3 read-back verification and retain seven completed sets. Recovered controller leader election, Deployment/Pod creation, scheduler binding and automatic node-certificate approval/registration also passed. A real kubelet also connected with the issued certificate, renewed its lease and ran one inert container in an isolated runtime. A [fresh lab worker replacement](../runbooks/restore-lab.md#worker-replacement-drill) also passed worker-only bootstrap, local volume checkpoint restoration, cross-node Cilium networking, DNS and egress policy checks. A [fresh control-plane VM](../runbooks/restore-lab.md#native-control-plane-recovery-drill) booted the original static pods from the complete offsite bundle and passed the same checks after reboot, with standalone kubelet preventing restored API workloads from running. The [two-node recovery drill](../runbooks/restore-lab.md#node-and-cilium-recovery-drill) then quarantined executable records in the recovered copy, registered fresh nodes and converged the production Cilium configuration. DNS, cross-node Service traffic, deny-egress enforcement and a lab L2 Gateway passed, including after node reboots. An [independent offsite volume restore](../runbooks/backup-and-restore.md#independent-fresh-volume-repeat) also recovered qBittorrent onto a fresh PVC using HCP S3 credentials and the repository password recovered from offsite etcd. The [combined drill](../runbooks/restore-lab.md#combined-machine-and-application-recovery-drill) then repeated that volume restore on two freshly replaced VMs with the recovered control plane/network and retained its application state through both node reboots. The backup inventory accounts for Velero warnings and unprotected data.
...
| IDs | Current assessment | Next step |
|---|---|---|
| K1, K4 | One control plane and single-voter Vault are single-instance dependencies. | [Phase 4](phase-4-compute-and-storage.md): three members and tested failover; a 2+1 placement across two hosts cannot survive loss of either host |
...
| K5, K16, K29, K31, K36 | Etcd snapshots and local/offsite Velero schedules are implemented, including distinct S3 prefixes and control-plane node-agent coverage. | Close the documented recovery coverage gaps, prove independent S3 credential recovery, and test backup-age/failure alerts in [Phase 1](phase-1-foundations.md) |
...
| K33, K41 | SQLite/native dumps bridge local-path application data into mounted NFS volumes for Velero. Recovery procedures include etcd tool compatibility and Vault/KMS and NFS bootstrap dependencies. | Inventory every application's data, test database/native archive consistency and restore retained state; set explicit recovery-point and recovery-time targets |
