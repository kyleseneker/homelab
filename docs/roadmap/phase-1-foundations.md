...
- [ ] Verify normal node registration and production Cilium convergence after the [fresh-machine static control-plane recovery](../runbooks/restore-lab.md#native-control-plane-recovery-drill); extend the verified [lab worker rebuild with checkpointed volumes](../runbooks/restore-lab.md#worker-replacement-drill) to offsite application-volume recovery after the [etcd/API, controller and kubelet/runtime checks](../runbooks/disaster-recovery.md#isolated-etcd-and-api-verification)
