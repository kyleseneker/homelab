...
This runbook walks through adding a new application to the cluster. All apps are deployed as ArgoCD Applications generated by an ApplicationSet. Each app has a `config.yml` (metadata), `values.yml` (Helm values), and optionally a `kustomization.yml` (supporting resources). The ApplicationSet discovers new apps automatically.
...
Create `config.yml` with the app metadata. This is what the ApplicationSet discovers.
...
Create `values.yml` with the Helm chart values.
...
!!! tip
    Use an existing app's `values.yml` as a starting point. The Sonarr values at `k8s/clusters/homelabk8s01/apps/arr/sonarr/values.yml` is a good reference for a typical arr-stack application.
...
2. Create a `kustomization.yml` listing the supporting resource:
...
3. Set `hasResources: true` in `config.yml`.
...
5. Reference the secret in `values.yml` via `envFrom` or `env.valueFrom`.
...
The ApplicationSet's Git File Generator automatically discovers the new `config.yml` and generates an Application. Monitor the deployment in the ArgoCD UI at `https://argocd.homelab.local`.
