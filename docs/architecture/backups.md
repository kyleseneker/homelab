...
| `vault/data-vault-0` | Live file-backend volume captured by weekly local/offsite Velero | Manual quiesced local archive restored with KMS auto-unseal; scheduled consistency/offsite recovery, authenticated reads and independent credentials remain unverified |
