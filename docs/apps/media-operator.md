...
Seerr currently references Sonarr/Radarr quality profile ID `7` with names `WEB-1080p` and `HD Bluray + WEB`. IDs belong to each application's database and may differ after a clean initialization. First run Recyclarr, query each app's `/api/v3/qualityprofile`, and confirm the IDs match the names before enabling household requests. Do not replace an ID with a guess. [Upstream PR #1](https://github.com/kyleseneker/media-operator/pull/1) adds name-based resolution and passed a lab check resolving the recreated Sonarr profile from stale ID `7` to ID `8`. It is merged but not released or deployed; the pinned operator still requires numeric IDs.
