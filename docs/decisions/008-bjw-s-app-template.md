...
Use the [bjw-s app-template](https://bjw-s-labs.github.io/helm-charts) Helm chart for general application workloads. Use purpose-built charts for operators and plain Kubernetes manifests for backup and configuration resources. Application-specific configuration is provided via a `values.yml` file in each app's directory, referenced by the ApplicationSet's multi-source template.
...
- **Consistent interface**: General application workloads share one value schema for containers, services, routes, persistence, and security contexts.
