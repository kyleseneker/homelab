...
- **Fairwinds VPA Helm chart**: A third-party packaging option. The SIG Autoscaling chart keeps the application and chart under the same upstream project's ownership.
...
- **Official VPA chart**: The chart at `https://kubernetes.github.io/autoscaler` is maintained by SIG Autoscaling alongside VPA, reducing reliance on separate downstream packaging.
...
- The Goldilocks dashboard is exposed at `https://goldilocks.homelab.local` via HTTPRoute. Its HTTPRoute uses the Authentik proxy-mode outpost.
