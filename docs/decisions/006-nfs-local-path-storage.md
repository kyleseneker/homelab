...
- **NFS Subdir External Provisioner** (`nfs-client` StorageClass): Default for shared service data; the media library uses a separately defined static NFS PV. Backed by the Unifi NAS with a Retain reclaim policy.
- **Rancher Local-Path Provisioner** (`local-path` StorageClass): Node-local storage for applications with SQLite WAL databases (including Sonarr and Radarr), plus Prometheus TSDB. Uses `WaitForFirstConsumer` binding and a Retain reclaim policy. Data stored at `/opt/local-path-provisioner/` on worker nodes.
...
- **local-path-provisioner only**: Simple and fast but ties PVCs to specific nodes with no failover. Media files would need to be on every node or use a shared mount. Used selectively where SQLite WAL shared-memory coordination or database storage requirements need a local filesystem.
...
- **Retain policy**: Data remains available after a claim is deleted. A replacement claim must be rebound to the retained volume or its existing storage path.
...
- NFS latency spikes can cause probe failures. Application health check timeouts must accommodate storage latency.
...
- Velero's Kopia file-system backup skips the `hostPath` volumes created by local-path-provisioner. Local-path data that must survive node loss therefore needs an application-level backup written onto an `nfs-client` volume for Velero to capture (ADR-013).
- The NAS is a single point of failure for NFS-backed data; local-path data instead shares the compute/VM disk failure domain. A backup strategy for persistent data is essential to mitigate this risk.
